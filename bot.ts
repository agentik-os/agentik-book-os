...
function systemPrompt(): string {
  ...
  return persona +
    ...
    `\n\n## CHANNEL\nYou are on Telegram (phone). Lead with the answer; chunked, bullet-friendly. For any structured diagram (a map, a graph, a tree) emit [[DIAGRAM: <mermaid code>]] on its own line — NEVER a "| title" (the | collides with Mermaid edge labels), NEVER ASCII, use <br/> not \\n inside labels. To attach a file, emit [[SEND: /absolute/path | caption]].`;
}
...
const MERMAID_CFG = `{"theme":"base","themeVariables":{"fontFamily":"Inter, system-ui, sans-serif","fontSize":"15px","primaryColor":"#ffffff","primaryTextColor":"#0a0a0a","primaryBorderColor":"#0a0a0a","lineColor":"#6b7280","secondaryColor":"#f4f4f4","tertiaryColor":"#fafafa","edgeLabelBackground":"#ffffff"},"flowchart":{"curve":"basis","padding":14,"nodeSpacing":45,"rankSpacing":55}}`;
...
function reportHtml(title: string, md: string, diagrams: Diagram[]): string {
  const bodyMd = md.replace(/^\s*#{1,2}\s+.*(?:\r?\n)+/, "");  // drop leading heading (rendered as H1)
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>${esc(title)}</title><style>${REPORT_CSS}</style></head><body><div class="wrap"><article class="sheet"><div class="brand">Agentik Book {OS}</div><h1 class="doc">${esc(title)}</h1><div class="rulebar"></div>${body}<div class="foot">Agentik Book {OS}</div></article></div></body></html>`;
}
